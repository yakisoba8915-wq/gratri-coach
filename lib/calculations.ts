...
export function calculateTrickRate(logs: PracticeLog[], trickId: string): number {
  return calculateSuccessRate(...logs.filter((log) => log.trickId === trickId).reduce<[number, number]>((sum, log) => [sum[0] + log.successCount, sum[1] + log.failCount], [0, 0]));
}
