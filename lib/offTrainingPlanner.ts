...
export function generateOffTrainingPlan(preferences:OffTrainingPreferences|LegacyPreferences,userId:string):OffTrainingPlan{
  const equipment=list(preferences.equipment);const locations=list(preferences.location);const focusAbilities=list(preferences.focusAbility);const targetTrickTypes=list(preferences.targetTrickType);const injuries=list(preferences.injuryConcern);const {intensity}=preferences;
  const hasGym=preferences.gymAvailable!=="使えない"&&(locations.includes("ジム")||preferences.gymAvailable==="使える");
  const knee=injuries.includes("膝")||injuries.includes("複数ある");
  const back=injuries.includes("腰")||injuries.includes("複数ある");
  const ankle=injuries.includes("足首")||injuries.includes("複数ある");
  const sets=intensity==="軽め"?2:intensity==="きつめ"?4:3;
  const pool:ExerciseSeed[]=[];

  if(equipment.includes("シバカツボードを持っている"))pool.push(exercise("シバカツ・プレス保持","シバカツ","左右30秒","プレス安定・板操作"),exercise("シバカツ・乗り系重心移動","シバカツ","左右8回","乗り系・重心移動"));
  if(equipment.includes("トリックスノーを持っている"))pool.push(exercise("トリックスノー・オーリー","シバカツ","8回","弾き"),exercise("トリックスノー・180","シバカツ","左右6回","回転力・乗り系"));
  if(!equipment.includes("シバカツボードを持っている")&&!equipment.includes("トリックスノーを持っている"))
    pool.push(exercise("ライン上プレス姿勢","シバカツ","左右30秒","プレス安定"),exercise("スケータージャンプ","シバカツ","左右8回","板操作・着地安定"));
  ...
  if(focusAbilities.includes("回転力")||targetTrickTypes.includes("360系")||targetTrickTypes.includes("540系"))pool.unshift(exercise("スタンディング回旋","シバカツ","左右10回","回転力"));
  if(focusAbilities.includes("体力"))pool.push(exercise("マウンテンクライマー","筋トレ","30秒","体力・体幹"));
  if(focusAbilities.includes("柔軟性"))pool.unshift(exercise("全身モビリティフロー","柔軟","5分","柔軟性"));
  ...
  const days=Array.from({length:preferences.weeklyDays},(_,index)=>({
    ...
    theme:index%2===0?`${focusAbilities.join("・")}を伸ばす日`:`${targetTrickTypes.join("・")}につなげる日`,
  ...
  return {id:`off-plan-${userId}`,title:`${focusAbilities.join("・")} × ${targetTrickTypes.join("・")} オフトレプラン`,description:`週${preferences.weeklyDays}日・1回${preferences.sessionMinutes}分を目安に、${intensity}の強度で取り組むプランです。`,weeklyDays:preferences.weeklyDays,sessionMinutes:preferences.sessionMinutes,days};
}
