...
環境変数がない場合、Supabaseへの読み書きが失敗した場合、または未ログインの場合は、ブラウザのlocalStorageを利用します。
...
ログイン中は各テーブルの `user_id` とRLSによりユーザーごとにデータが分離されます。ログアウトするとゲスト用localStorageへ戻ります。既存の認証前データは自動ではユーザーへ移行されません。
