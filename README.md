...
- プロフィール画面から、不具合報告・改善要望・機能提案・その他のフィードバックを送信できます。
- SQL Editorで `supabase/user-feedback.sql` を実行し、`user_feedback` テーブルとRLSポリシーを追加してください。
- ユーザーは自分のフィードバックのみ insert / select でき、update / delete はできません。
- プロフィール画面では、自分が送信したフィードバックの種類・対象画面・内容・重要度・ステータス・送信日を確認できます。
- 未ログイン時はフィードバックを送信できません。

## 初回起動チュートリアル

- 初回アクセス時は、Gratri Coachの主要機能を紹介する5枚のチュートリアルを表示します。
- 表示済み状態はブラウザのlocalStorageに `gratri_onboarding_seen` として保存します。
- 「スキップ」または最終画面の「はじめる」で完了できます。
- 未ログインでも表示され、チュートリアル終了後に既存のログイン促進モーダルを表示します。

## ランディングページ

- 未ログインかつゲスト閲覧を開始していないユーザーには、トップページ `/` でランディングページを表示します。
- ランディングページでは、AIグラトリコーチの価値、機能紹介、無料範囲、Premium予定、βテスター案内、使い方を表示します。
- 「無料で始める」「Googleでログイン」は既存のGoogleログイン処理を使用します。
- 「ログインせずに見る」を押すと `localStorage` に `gratri_guest_mode = true` を保存し、既存のホーム画面へ進みます。
- ランディングページ表示中は、既存の初回チュートリアル、ログイン促進モーダル、ホームタブ初回説明が重ならないようにしています。

## 各タブの初回説明

- ホーム、トリック、練習、オフトレ、AI対話、プロフィールを初めて開いたときだけ、小型の説明モーダルを表示します。
...
- シバカツトリックにも通常トリックと同じ `stance` を持たせ、基本は `both` として扱います。

## 技スキルツリー

- 通常トリック画面の技ツリーボタンから、前提技と派生技を線で結んだスキルツリーを開けます。
- プレス基礎、オーリー系、ノーリー系、応用・派生の小さなツリーに分割し、モバイルでは縦方向にたどれます。
- DB追加の通常トリックは、`prerequisite` が既存技と一致すればその技の子ノードとして表示します。
- 前提技が未設定または一致しないDB追加技は「その他」へ表示します。
- シバカツトリックは通常トリックの技ツリーには含めません。
- トリック一覧と技ツリーでは「レギュラー / グーフィー」の表示スタンスを切り替えられます。
- 表示スタンスは説明・コツ・表示ラベルを切り替えるための状態で、一覧の表示除外には使いません。初期20技、DB追加技、シバカツトリックはどちらの表示でも全件表示します。
- プロフィールのスタンスは初期表示の選択にだけ使います。未ログインまたは未設定時はレギュラー表示から開始します。
- 画面上の技名は `lib/trickDisplay.ts` の `formatTrickName()` で、選択中の表示スタンスに応じて `バックノーズプレス（レギュラー）` / `バックノーズプレス（グーフィー）` のように表示します。
- DB上の `name` / `nameJa`、検索、保存、AI入力、練習記録の `trickId` は従来どおり元の技名・IDを使用し、括弧付き表示名は保存しません。

## DBシバカツメニューによる週間プラン生成

- オフトレ診断完了時に、公開RLSを通して `tricks` テーブルの `trick_type = 'shibakatsu'` を取得します。
- Q6の伸ばしたい能力、Q7の技系統、希望強度と難易度を採点し、週間プランの「シバカツの日」に1日2〜3メニューを配置します。
