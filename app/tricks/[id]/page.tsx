...
export default function TrickDetailPage({ params }: { params:Promise<{ id:string }> }) {
  const { id } = use(params); const [storedTricks,refresh] = useLocalData(localData.getTricks); const [storedLogs] = useLocalData(localData.getLogs);
  ...
  function updateFavorite() { localData.saveTricks(tricks.map((item)=>item.id===id ? {...item,favorite:!item.favorite}:item)); refresh(); setSaved(true); setTimeout(()=>setSaved(false),1000); }
  function updateStatus(value:string) { if (!masteryStatuses.includes(value as (typeof masteryStatuses)[number])) return; localData.saveTricks(tricks.map((item)=>item.id===id ? {...item,masteryStatus:value as (typeof masteryStatuses)[number]}:item)); refresh(); }
