...
export default function TrickDetailPage({ params }: { params:Promise<{ id:string }> }) {
  ...
  return <main><PageHeader title={trick.nameJa} eyebrow={`${trick.category} ・ LV.${trick.difficulty}`} back="/tricks"/>
    ...
    <section className="card mb-4"><div className="flex items-center justify-between"><h2 className="font-black">自分の成功率</h2><strong className="text-2xl text-glacier">{user?`${rate}%`:"—"}</strong></div>{user?<ProgressChart logs={trickLogs}/>:<div className="grid h-24 place-items-center text-sm text-slate-400">ログイン後に表示されます</div>}</section>
