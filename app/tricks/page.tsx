...
export default function TricksPage() {
  const { user } = useAuth();
  const [stored, refresh] = useSupabaseData(dataRepository.getTricks);
    ...
    <main>
      <div className="flex items-start justify-between gap-3">
        ...
        <button type="button" onClick={() => setAddModalOpen(true)} className="btn-primary shrink-0 !px-3 !py-3 text-sm">
          <Plus size={18} />
          <span className="hidden sm:inline">技を追加</span>
        </button>
      </div>

      {created && <p className="mb-4 rounded-2xl bg-emerald-50 px-4 py-3 text-sm font-bold text-emerald-700">技を追加しました</p>}

      <div className="relative mb-3">
        <Search className="absolute left-4 top-3.5 text-slate-400" size={18} />
        <input className="field pl-11" placeholder="技名で検索" value={query} onChange={(event) => setQuery(event.target.value)} />
      </div>
      ...
      <AddTrickModal open={addModalOpen} onClose={() => setAddModalOpen(false)} onCreated={handleCreated} />
    </main>
  );
}
