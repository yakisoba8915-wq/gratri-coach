...
export default function ProfilePage() {
  const {user}=useAuth();
  const [stored,refresh]=useSupabaseData(dataRepository.getProfile); const [storedTricks]=useSupabaseData(dataRepository.getTricks); const [storedLogs]=useSupabaseData(dataRepository.getLogs); const profile=stored??initialProfile; const [displayName,setDisplayName]=useState(""); const [stance,setStance]=useState<Stance|"">(""); const [saved,setSaved]=useState(false);
  ...
  async function save(){if(!user||!stance)return;await dataRepository.saveProfile({displayName,stance});await refresh();setSaved(true);setTimeout(()=>setSaved(false),1200)}
  ...
  return <main><PageHeader title="プロフィール" eyebrow="MY PAGE"/><div className="mb-5 flex items-center gap-4"><div className="grid h-20 w-20 place-items-center rounded-3xl bg-gradient-to-br from-cyan-200 to-blue-200 text-2xl font-black text-glacier">{user?(profile.displayName.slice(0,1)||"—"):"—"}</div><div><h2 className="min-h-7 text-xl font-black">{user?profile.displayName:""}</h2><p className="mt-1 min-h-4 text-xs font-bold text-slate-400">{user&&profile.stance?`${profile.stance}スタンス`:""}</p></div></div><div className="mb-5 flex gap-3"><StatCard label="練習記録" value={logCount} suffix="件"/><StatCard label="完成トリック" value={completed} suffix="技"/></div>
    <section className="card mb-4 grid gap-4"><h2 className="font-black">基本情報</h2><label className="text-sm font-bold">表示名<input disabled={!user} className="field mt-2 disabled:bg-slate-50" value={user?displayName:""} onChange={(e)=>setDisplayName(e.target.value)}/></label><label className="text-sm font-bold">スタンス<select disabled={!user} className="field mt-2 disabled:bg-slate-50" value={user?stance:""} onChange={(e)=>setStance(e.target.value as Stance|"")}><option value="">未選択</option><option>レギュラー</option><option>グーフィー</option></select></label><button disabled={!user||!stance} onClick={save} className="btn-primary disabled:bg-slate-200 disabled:text-slate-400"><Save size={17}/>{!user?"ログイン後に保存できます":saved?"保存しました":"プロフィールを保存"}</button></section>
