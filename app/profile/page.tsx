...
import { ChevronRight, LogIn, Save, Target, Trees } from "lucide-react";
...
export default function ProfilePage() {
  const [stored,refresh]=useLocalData(localData.getProfile); const profile=stored??initialProfile; const [displayName,setDisplayName]=useState(profile.displayName); const [stance,setStance]=useState<Stance>(profile.stance); const [saved,setSaved]=useState(false);
  function save(){localData.saveProfile({displayName,stance});refresh();setSaved(true);setTimeout(()=>setSaved(false),1200)}
  const completed=localData.getTricks().filter((t)=>t.masteryStatus==="完成").length; const logCount=localData.getLogs().length || initialPracticeLogs.length;
  return <main><PageHeader title="プロフィール" eyebrow="MY PAGE"/><div className="mb-5 flex items-center gap-4"><div className="grid h-20 w-20 place-items-center rounded-3xl bg-gradient-to-br from-cyan-200 to-blue-200 text-2xl font-black text-glacier">{profile.displayName.slice(0,1)}</div><div><h2 className="text-xl font-black">{profile.displayName}</h2><p className="mt-1 text-xs font-bold text-slate-400">{profile.stance}スタンス</p></div></div><div className="mb-5 flex gap-3"><StatCard label="練習記録" value={logCount} suffix="件"/><StatCard label="完成トリック" value={completed} suffix="技"/></div>
    ...
    <div className="card flex items-center gap-3 text-slate-400"><LogIn/><div><p className="font-bold">Googleログイン</p><p className="text-xs">Supabase連携時に利用可能になります</p></div><span className="ml-auto rounded-full bg-slate-100 px-2 py-1 text-[10px] font-bold">準備中</span></div></main>;
}
