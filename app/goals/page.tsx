...
export default function GoalsPage() {
  const [stored,refresh]=useLocalData(localData.getGoals); const goals=stored??initialGoals; const [open,setOpen]=useState(false); const [season,setSeason]=useState("2026-27"); const [type,setType]=useState<GoalType>("技をメイクする"); const [trickId,setTrickId]=useState(initialTricks[0].id); const [targetRate,setTargetRate]=useState(50);
  function save(){const goal:Goal={id:`goal-${Date.now()}`,season,type,trickId,completed:false,...(type==="成功率を上げる"?{targetRate}:{})};localData.saveGoals([...goals,goal]);refresh();setOpen(false)}
  function update(id:string,patch:Partial<Goal>){localData.saveGoals(goals.map((g)=>g.id===id?{...g,...patch}:g));refresh()} function remove(id:string){localData.saveGoals(goals.filter((g)=>g.id!==id));refresh()}
