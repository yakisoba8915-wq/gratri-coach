...
export async function PATCH(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  if (existingError) return NextResponse.json({ error: "技データを確認できませんでした。" }, { status: 500 });
  if (!existing) return NextResponse.json({ error: "対象の技が見つかりません。" }, { status: 404 });

  const trickType = normalizeTrickType(body.trickType ?? existing.trick_type);
  if (!trickType) return NextResponse.json({ error: "入力内容を確認してください。" }, { status: 400 });

  const name = body.name?.trim() ?? String(existing.name ?? "");
  const difficulty = body.difficulty === undefined ? Number(existing.difficulty) : Number(body.difficulty);
  const category = body.category ?? String(existing.category ?? "");
  const takeoffType = body.takeoffType ?? String(existing.takeoff_type ?? "なし");
  const spinDirection = body.spinDirection ?? String(existing.spin_direction ?? "なし");
  const stance: TrickStance = body.stance === undefined || body.stance === "" ? (existing.stance ?? "both") as TrickStance : body.stance as TrickStance;
  const accessTypeValue = body.accessType ?? body.access_type ?? existing.access_type;
  ...
  if (trickType === "snow" && initialTricks.some((trick) => normalized(trick.nameJa) === normalized(name))) {
    return NextResponse.json({ error: "初期トリックと同名の技は登録できません。" }, { status: 409 });
  }
  ...
  const { data: duplicate, error: duplicateError } = await adminClient
    ...
    .neq("id", id)
    .limit(1);
  if (duplicateError) return NextResponse.json({ error: "技データを確認できませんでした。" }, { status: 500 });
  if (duplicate && duplicate.length > 0) return NextResponse.json({ error: "同じ種類に同名の技がすでに登録されています。" }, { status: 409 });
  ...
  const { data, error } = await adminClient
    ...
    .update({
      ...
      tips: body.tips?.trim() ?? existing.tips ?? "",
      prerequisite: trickType === "snow" ? body.prerequisite?.trim() ?? existing.prerequisite ?? "" : "",
      trick_type: trickType,
      stance,
      access_type: accessType,
      related_snow_trick: trickType === "shibakatsu" ? body.relatedSnowTrick?.trim() ?? existing.related_snow_trick ?? "" : "",
      cautions: trickType === "shibakatsu" ? body.cautions?.trim() ?? existing.cautions ?? "" : "",
    })
    .eq("id", id)
    .select("*")
    .single();

  if (error) return NextResponse.json({ error: "技の更新に失敗しました。" }, { status: 500 });
...
export async function DELETE(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  if (error) return NextResponse.json({ error: "技の削除に失敗しました。" }, { status: 500 });
