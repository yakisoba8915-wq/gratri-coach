...
export default function AddTrickModal({ open, onClose, onCreated }: AddTrickModalProps) {
  const [name, setName] = useState("");
  const [difficulty, setDifficulty] = useState(1);
  const [category, setCategory] = useState<(typeof categories)[number]>("プレス系");
    ...
    <div className="fixed inset-0 z-[100] grid place-items-end bg-navy/45 p-4 backdrop-blur-sm sm:place-items-center" role="dialog" aria-modal="true" aria-labelledby="add-trick-title">
      <div className="max-h-[92vh] w-full max-w-md overflow-y-auto rounded-[2rem] bg-white p-6 shadow-2xl">
        <div className="flex items-start justify-between gap-4">
          <div>
            <h2 id="add-trick-title" className="text-xl font-black">技を追加</h2>
        ...
        <div className="mt-5 grid gap-4">
          <label className="text-sm font-bold">技名 <span className="text-rose-500">*</span><input autoFocus className="field mt-2" value={name} onChange={(event) => setName(event.target.value)} /></label>
          ...
          <label className="text-sm font-bold">系統<select className="field mt-2" value={category} onChange={(event) => setCategory(event.target.value as (typeof categories)[number])}>{categories.map((item) => <option key={item}>{item}</option>)}</select></label>
