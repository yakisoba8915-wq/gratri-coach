...
import { ChevronRight, Heart, Lock, Pencil } from "lucide-react";
...
export default function TrickCard({
  ...
  if (!canUse) {
    return (
      <>
        <article className={`card border-amber-100 bg-amber-50/45 opacity-80 ${compact ? "!rounded-2xl !p-3" : ""}`}>
          <button type="button" onClick={() => setPremiumOpen(true)} className="group block w-full text-left transition active:scale-[.99]">
  ...
  return (
    <article className={`card ${compact ? "!rounded-2xl !p-3" : ""}`}>
      <Link href={`/tricks/${trick.id}?stance=${selectedStance}`} className="group block">
        <div className="flex items-center gap-3">
